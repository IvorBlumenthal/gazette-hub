...
function renderEmailHtml(issue, unsubscribeUrl) {
  const sections = (issue.sections || []).map(function (s) {
    const notices = (s.notices || []).map(function (n) {
      const link = n.source_url ? ' — <a href="' + escHtml(n.source_url) + '">view notice</a>' : '';
      return '<li>' + escHtml(n.title) + (n.date ? ' (' + escHtml(n.date) + ')' : '') + link + '</li>';
    }).join('');
  ...
  return '<div style="font-family:Segoe UI,system-ui,sans-serif;max-width:600px;margin:0 auto">'
    ...
    + 'ArkKonsult PTY Limited &middot; <a href="' + escHtml(unsubscribeUrl) + '" style="color:#8A8A8A">Unsubscribe</a>'
    + '</div>'
    + '</div>';
}

module.exports = { renderEmailHtml, escHtml };
